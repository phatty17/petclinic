...
- [ ] 2.2 Extract a `buildSortOrder(sortParam?: string)` pure helper (equivalent QueryBuilder `.orderBy` calls) and implement so all tests pass; throw `BadRequestException` for any column outside `['lastName', 'city', 'address']` or direction outside `['asc', 'desc']`
...
- [ ] 7.7 Name cell bold-on-sort cue: keep rendering `firstName lastName`, but wrap the lastName portion in a `<strong>` (or bold-styled `<span>`) shown only while the name sort is active (`sort.active === 'lastName'`); plain rendering when sort is inactive or on another column
